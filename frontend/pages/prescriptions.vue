    ...
    <div class="space-y-6">
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="bg-gray-800 p-4 rounded-xl border border-gray-700">
          <p class="text-sm text-gray-400">Total Prescriptions</p>
          <p class="text-2xl font-bold text-white mt-1">{{ prescriptions.length }}</p>
        </div>
          ...
          <table class="w-full">
            <thead class="bg-gray-700/50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-300">Medication</th>
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-300">Dosage</th>
            ...
            <tbody class="divide-y divide-gray-700">
              ...
              >
                <td class="px-6 py-4 text-sm text-gray-300 font-mono">{{ presc.id }}</td>
                <td class="px-6 py-4 text-sm text-gray-300">{{ presc.patient_id }}</td>
                <td class="px-6 py-4 text-sm font-medium text-white">{{ presc.medication }}</td>
                <td class="px-6 py-4 text-sm text-gray-300">{{ presc.dosage }} ({{ presc.duration }})</td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4 text-sm text-gray-400">{{ presc.created_at ? new Date(presc.created_at).toLocaleDateString() : 'Unknown' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
