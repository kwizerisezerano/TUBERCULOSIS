<template>
  <DashboardLayout page-title="Diagnoses">
    <div class="space-y-6">
      <div class="flex flex-col sm:flex-row gap-4 justify-between items-center">
        <h2 class="text-xl font-bold text-white">Diagnosis Records</h2>
          ...
          <table class="w-full">
            <thead class="bg-gray-700/50">
              <tr>
                ...
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-300">Patient ID</th>
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-300">Diagnosis Type</th>
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-300">Risk Level</th>
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-300">Confidence</th>
                <th class="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-300">Status</th>
            ...
            <tbody class="divide-y divide-gray-700">
              ...
              >
                <td class="px-6 py-4 text-sm text-gray-300 font-mono">{{ diagnosis.id }}</td>
                <td class="px-6 py-4 text-sm text-gray-300">{{ diagnosis.patient_id }}</td>
                <td class="px-6 py-4 text-sm font-medium text-white">{{ diagnosis.diagnosis_type }}</td>
                <td class="px-6 py-4">
                  <span :class="[
                    'px-3 py-1 rounded-full text-xs font-semibold',
                    getRiskLevelClass(diagnosis.risk_level)
                  ]">
                ...
                <td class="px-6 py-4 text-sm text-gray-400">{{ diagnosis.created_at ? new Date(diagnosis.created_at).toLocaleDateString() : 'Unknown' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const getRiskLevelClass = (riskLevel: string | null) => {
  ...
  if (level.includes('high')) return 'bg-red-900/30 text-red-400';
  if (level.includes('moderate') || level.includes('medium')) return 'bg-yellow-900/30 text-yellow-400';
  if (level.includes('low') || level.includes('minimal')) return 'bg-green-900/30 text-green-400';
